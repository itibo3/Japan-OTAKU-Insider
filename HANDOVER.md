...
**作成日:** 2026-03-16 (最終更新: 2026-03-23)  
**ステータス:** β版完成 / 独自ドメイン（otaku.eidosfrontier.com）移行完了 / HTTPS有効 / AdSense審査待ち  
**開発期間:** 約6時間  
...
│   ├── check_deepl_quota.py # DeepL残枠監視＆ログ保存スクリプト
...
- **2026-03-31 (DeepL API枠監視・X連携によるレポート通知機能)**
...
- **2026-03-27 (EN/JP言語切替機能・パイプライン追加):**
  ...
  **コミット: `de28a05`** — "feat: EN/JPトグル実装・entries_ja.json自動生成パイプライン追加"
  ...
  | `data/entries_ja.json` | `title_ja` を `title` に適用した日本語版JSON（1447件） |
...
- **2026-03-22 (RSS Manager 全ソース監査 + 緊急バグ修正 3件 + sources.json 実態修正):**
  ...
  **RSS Manager 現状の残課題（ROADMAP より）:**
  ...
  - Perplexity 品質チェック / Gemini Flash 連携（未実装）
...
- **2026-03-19 (RSS・サムネイル・UI 一括修正):**
  ...
  - サムネイル品質改善:
    - `skip_keywords` に `ico_`, `common/img`, `shared/img`, `noimage`, `no_image`, `dummy`, `sns_icon` を追加（rss_fetch.py, fill_og_images.py 両方）。
...
1. **DeepL APIキーを Secrets に設定 → Actions で翻訳実動確認**
   - リポジトリ設定 → Secrets → `DEEPL_AUTH_KEY` を設定
   - Actions 手動トリガ → 翻訳ステップが動くことを確認
...
1. **DeepL APIキー設定 → 翻訳自動化**（スクリプトは完成、キー設定のみ残り）
2. **RSS Manager B2 完成**（articles→entries.json パイプライン、DELETE API）
3. **記事削除ワークフロー**（GitHub Actions workflow_dispatch で ID 指定削除）
4. **Perplexity品質チェック**（Gemini Flash API 連携）
5. **週間サバイバルレポート**（GitHub Issues 自動投稿）
6. 将来：ミニPC/ラズパイ＋ローカルLLM環境ができたら、DeepL/検索API部分だけ差し替えて自前運用へ
