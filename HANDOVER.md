...
**作成日:** 2026-03-16 (最終更新: 2026-04-14)  
...
- **2026-04-14（週次ループ: Opus 優先でバラデータ統合解釈）:**

  **内容:**
  - 補足: 当初 Sonnet を先にしていたのは **API キー疎通テストで失敗を繰り返す前提で Opus を無駄にしたくなかった**ため。本番方針はもともと **Opus 4.6 系**で統合解釈すること。
  - `scripts/weekly_self_improve_loop.py`: 週次レポート・Perplexity 改善案・JOI 素材生成の **第一モデルを既定で Opus**（失敗時 Sonnet）。`--weekly-llm-order opus_first|sonnet_first` および環境変数 `ANTHROPIC_WEEKLY_LLM_ORDER` で切替。
  - `REPORT_SYSTEM` / `CLAUDE_SYSTEM`: チャネル横断で読み、粒度の違う指標に注釈を付けてよい旨と、週報を Perplexity 検索意図のヒントに使う旨を追記。
  - JOI の `ensure_joi_english_fields` は **Sonnet 固定**（英語補完のみで Opus 連打を避ける）。
  - `.github/workflows/weekly-self-improve.yml`: 上記順序を引数で渡す。Repo Variables `ANTHROPIC_WEEKLY_LLM_ORDER` 任意。
