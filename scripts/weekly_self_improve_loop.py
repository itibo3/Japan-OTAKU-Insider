...
与えられた統計JSONはチャネルがバラバラでもよい（例: サイト内集計・GA4の合計値・パイプライン内部指標。将来はX投稿メトリクスやページ別PVのブロックが増える想定）。
あなたの仕事は、これらを**横断的に読み比べ、一枚岩の週次解釈**にまとめること。粒度や意味が違う指標を無理に一列に並べず、「何が言えて何が言えないか」を短く注釈してよい。
週次の振り返りレポートを日本語Markdownで書いてください。
必ず次を含めてください:
- 先週の要点（3点以内）
- 良かった点 / 悪化点
...
You receive a Japanese weekly report that may synthesize multiple data sources (on-site stats, GA4 aggregates, pipeline health; later possibly X post performance or page-level traffic). Use that synthesis as **hints for search intent and topics readers actually care about**, without inventing facts not supported by the report.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Opus モデル（opus_first 時は第一候補、sonnet_first 時はフォールバック）",
    )
    parser.add_argument(
        "--sonnet-model",
        default=(os.getenv("ANTHROPIC_SONNET_MODEL", "").strip() or DEFAULT_SONNET_MODEL),
        help="Sonnet モデル（sonnet_first 時は第一候補、opus_first 時はフォールバック）",
    )
    ...
    # 週次分析レポート（既定: Opus 優先 = チャネル横断の統合解釈。失敗時に Sonnet へ）
    report_user = "統計JSON:\n" + json.dumps(stats, ensure_ascii=False, indent=2)
    try:
        llm_trace["report_attempts"].append({"model": report_primary, "status": "try"})
        ...
        print(f"週次レポート第一モデル失敗 ({e}); {report_fallback} へフォールバック", file=sys.stderr)
        llm_trace["report_attempts"].append({"model": report_fallback, "status": "try"})
        report = call_anthropic_text(
            api_key=anthropic_key,
    ...
    # 検索ワード改善案（レポートと同じ第一／フォールバック順）
    try:
        llm_trace["proposal_attempts"].append({"model": report_primary, "status": "try"})
        proposals = call_claude_json(anthropic_key, report_primary, report_text, prompts)
        ...
        print(f"改善案第一モデル失敗 ({e}); {report_fallback} へフォールバック", file=sys.stderr)
        llm_trace["proposal_attempts"].append({"model": report_fallback, "status": "try"})
        proposals = call_claude_json(anthropic_key, report_fallback, report_text, prompts)
        llm_trace["proposal_attempts"][-1]["status"] = "ok"
